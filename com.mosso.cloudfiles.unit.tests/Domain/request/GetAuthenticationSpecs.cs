...
namespace com.mosso.cloudfiles.unit.tests.Domain.request.GetAuthenticationSpecs
{
    ...
    public class when_authenticating_against_cloud_files_and_no_auth_url_provided
    {
        ...
        {
            Assert.That(getAuthentication.Uri.ToString(), Is.EqualTo(utils.Constants.MOSSO_AUTH_URL));
        }
        ...
        {
            Assert.That(getAuthentication.Headers[utils.Constants.X_AUTH_USER], Is.EqualTo("username"));
        }

        [Test]
        public void should_have_api_key_in_the_headers()
        {
            Assert.That(getAuthentication.Headers[utils.Constants.X_AUTH_KEY], Is.EqualTo("apikey"));
        }
    }
